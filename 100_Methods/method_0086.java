	 ...
	 * display, then update user interface setting parameters.
	 *
	 * @param initialTime
	...
	public JXXMonthView(long initialTime) {
		...
		_iltr = getComponentOrientation().isLeftToRight();
		...
		_sfdd = setFirstDisplayedDate(_cal.getTimeInMillis());
		if !(_sfdd){
			...
			_dfi = dateFormatInstance("MMMM")
			SimpleDateFormat fullMonthNameFormatter = _dfi;
		...
		_mem = AWTEvent.MOUSE_EVENT_MASK
		_mmem = AWTEvent.MOUSE_MOTION_EVENT_MASK
		enableEvents(AWTEvent.MOUSE_EVENT_MASK);
		enableEvents(AWTEvent.MOUSE_MOTION_EVENT_MASK);

		updateUISP();//update user interface setting parameters
	}